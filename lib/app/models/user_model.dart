...
class User extends Model {
  ...
  PhoneNumber getPhoneNumber() {
    if (this.phoneNumber != null && this.phoneNumber!.length > 4) {
      String phoneValue = this.phoneNumber!.replaceAll(' ', '');
      String dialCode1 = phoneValue.substring(1, 2);
      String dialCode2 = phoneValue.substring(1, 3);
      String dialCode3 = phoneValue.substring(1, 4);
      for (int i = 0; i < countries.length; i++) {
        if (countries[i].dialCode == dialCode1) {
          return new PhoneNumber(countryISOCode: countries[i].code, countryCode: dialCode1, number: phoneValue.substring(2));
        } else if (countries[i].dialCode == dialCode2) {
          return new PhoneNumber(countryISOCode: countries[i].code, countryCode: dialCode2, number: phoneValue.substring(3));
        } else if (countries[i].dialCode == dialCode3) {
          return new PhoneNumber(countryISOCode: countries[i].code, countryCode: dialCode3, number: phoneValue.substring(4));
        }
      }
    }
    return new PhoneNumber(countryISOCode: Get.find<SettingsService>().setting.value.defaultCountryCode ?? 'US', countryCode: '1', number: '');
  }

  bool get isAdmin => (this.roles.where((element) => element.name == 'admin').length) > 0;

  bool get isProvider => (this.roles.where((element) => element.name == 'provider' || element.name == 'admin').length) > 0;

  bool get isCustomer => (this.roles.where((element) => element.isdDefault).length) > 0;
